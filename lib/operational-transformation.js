var operational_transformation = (function () {
  ...
  function Operation (baseRevision, id, ops) {
    assert(
      typeof baseRevision === 'number' && baseRevision >= 0,
  ...
  Operation.prototype.retain = function (n) {
    assert(n > 0, "num of retained characters must be greater than 0");
    this.baseLength += n;
    this.targetLength += n;
  ...
  Operation.prototype.insert = function (str) {
    assert(str, "inserted string mustn't be the empty string");
  ...
  Operation.prototype.delete = function (str) {
    assert(str, "deleted string mustn't be the empty string");
  ...
  Operation.fromJSON = function (obj) {
    assert(obj.id);
    var o = new Operation(obj.baseRevision, obj.id);
    var ops = obj.ops;
  ...
  function apply (str, operation) {
    if (str.length !== operation.baseLength) {
      throw new Error("The operation's base length must be equal to the strings length.");
    }
    var newStr = [], j = 0;
    var strIndex = 0;
