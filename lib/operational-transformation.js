var operational_transformation = (function () {
  ...
  Operation.prototype.skip = function (n) {
    assert(n > 0, "num of skipped characters must be greater than 0");
    this.baseLength += n;
    this.targetLength += n;
    var lastOp = this.ops[this.ops.length-1];
    if (lastOp && lastOp.skip) {
      lastOp.skip += n;
    } else {
  ...
  function compose (operation1, operation2) {
    ...
    while (true) {
      var op1l = op1 && (op1.skip || (op1.insert || op1.delete).length);
      var op2l = op2 && (op2.skip || (op2.insert || op2.delete).length);
