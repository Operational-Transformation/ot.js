...
ot.CodeMirrorServer = (function (global) {
  ...
  var WrappedOperation = global.ot ? globla.ot.WrappedOperation : require('./wrapped-operation');
  ...
  CodeMirrorServer.prototype.onOperation = function (socket, obj) {
    var operation;
    try {
      operation = TextOperation.fromJSON(obj.operation);
    } catch (exc) {
    ...
    try {
      ...
      socket.broadcast.emit('operation', {
        meta: meta,
        operation: operationPrime.toJSON()
      });
    } catch (exc) {
      console.error(exc);
    }
